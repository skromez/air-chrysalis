...
export let giveaway: Giveaway;
export let address: string;
</script>

<Card class="m-4 rounded-2xl">
	<Content>ID: {giveaway.id}</Content>
	...
	<Button
		ripple="{false}"
		on:click="{() => goto(`/${address}/${giveaway.id}`)}"
		class="button-shaped-round m-4"
		variant="raised">Go to giveaway</Button
	>
</Card>
