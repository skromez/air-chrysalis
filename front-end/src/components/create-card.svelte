...
<Card class="m-4 p-3 rounded-2xl w-full">
	...
	>
		<div class="flex justify-center items-center">
			<Button
				disabled="{card.selectedAmount === 1}"
				on:click="{decrease}"
				ripple="{false}"
				variant="raised"
			...
			<Button
				disabled="{card.selectedAmount === card.amount}"
				on:click="{increase}"
				ripple="{false}"
				variant="raised"
				class="button-shaped-round min-w-[12px] w-[24px] h-[25px]">+</Button
		...
		<Button
			...
			ripple="{false}"
			variant="raised"
			class="text-xs button-shaped-round min-w-[42px] w-[42px] h-[25px] normal-case">Max</Button
		>
	</Content>
</Card>
