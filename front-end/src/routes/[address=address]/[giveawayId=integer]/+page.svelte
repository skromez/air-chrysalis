...
const sendPrizes = async () => {
	...
	for (const winner of winners) {
		...
		const data = erc1155Interface.encodeFunctionData("safeBatchTransferFrom", [
			$auth.address,
			winner.address,
			tokenIds,
			tokenAmounts,
	...
	try {
		const data = contractInterface.encodeFunctionData("prizeSent", [address, giveawayId]);
...
const fetchCards = async () => {
	const tokensAmountMap = new Map(
		giveaway.tokenTuples.map((tuple) => [tuple[0].toString(), tuple[1].toNumber()])
	);
	...
	if (giveaway.tokenTuples.length > 50) {
		...
		let metadata = await MetadataService.getMetadata(tokenIds);
		metadata = await MetadataService.getMetadata(tokenIds);
...
const itemClick = async (card: SkyweaverCard) => {
	if (isItemIsCard(card)) {
		const url = `https://play.skyweaver.net/items/cards/${card.properties.baseCardId}/${
			card.properties.type === "Gold" ? "gold" : "silver"
		}`;
		window.open(url, "_blank");
	}
};
...
<div class="mx-auto max-w-[720px] flex flex-col mt-4">
	{#if loadingVRFVerification}
		<div class="w-full flex flex-col justify-center items-center">
			<span class="text-2xl"> Waiting for Chainlink VRF Randomizer... </span>
			...
			<Card class="p-3 rounded-2xl w-full mt-4">
				...
				<Content>
					<div>
						...
						{:else}
							<div
								class="grid mt-4 w-full max-h-[175px] overflow-y-auto grid-flow-row grid-cols-[repeat(auto-fill,_120px)] justify-center justify-items-center"
							>
								{#each cards as card}
									<div
										class:cursor-pointer="{isItemIsCard(card)}"
										on:click="{() => itemClick(card)}"
		...
		<div>
			<Container>
				{#if $auth.connected && giveaway && giveaway.finished && address.toLowerCase() === $auth.address.toLowerCase() && !giveaway.prizeSent}
					<Button
						ripple="{false}"
						class="button-shaped-round w-full py-6 mt-3"
						on:click="{sendPrizes}"
						variant="raised"
					>
						Send Prize
				...
				{#if $auth.connected && giveaway && address.toLowerCase() === $auth.address.toLowerCase() && !giveaway.finished}
					{#if !giveaway.finished}
						<div class="flex gap-2">
							<Button
								ripple="{false}"
								class="button-shaped-round w-full py-6 mb-4"
					...
					<Button
						ripple="{false}"
						class="button-shaped-round w-full py-6"
						on:click="{finishGiveaway}"
						variant="raised"
						>{giveaway.participants.length < giveaway.winnersAmount ? "Cancel" : "Finish"}
						Giveaway
					</Button>
				{/if}
