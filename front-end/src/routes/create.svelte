<script lang="ts">
  ...
  let maxWinnersAmount = $cards.filter((card) => card.selected).reduce((prev, curr) => {
    return prev + curr.selectedAmount
  }, 0)

  const calcMaxWinnersAmount = () => {
    maxWinnersAmount = $cards.filter((card) => card.selected).reduce((prev, curr) => {
      return prev + curr.selectedAmount
    }, 0)
  }
...
{#if !loading}
  <div class="mx-auto max-w-[720px] flex flex-col items-center">
    {#each $cards.filter((card) => card.selected) as card (card.tokenId)}
      <CreateCard on:changeAmount={calcMaxWinnersAmount} on:removeCard={onRemoveCard} card={card}/>
    {/each}
    {#if $cards.filter((card) => card.selected).length}
      <Button disabled={$cards.every((card) => card.selected)} on:click={async () => await goto('/assets')} ripple={false} class="button-shaped-round w-full py-6 normal-case text-base" variant="raised">Add more cards</Button>
      <div class="m-4 rounded-2xl w-full">
        <Content class="rounded-2xl border-des-purple border-solid border flex justify-between items-center">
          ...
          <div class="flex justify-center items-center">
            ...
            {/if}
            <Button disabled={winnersAmount === 1} ripple={false} on:click={() => winnersAmount--} variant="raised" class="button-shaped-round min-w-[12px] w-[24px] h-[25px]">-</Button>
            ...
            <Button disabled={winnersAmount === maxWinnersAmount} ripple={false} on:click={() => winnersAmount++} variant="raised" class="button-shaped-round min-w-[12px] w-[24px] h-[25px]">+</Button>
            <Button disabled={winnersAmount === maxWinnersAmount} ripple={false} on:click={setWinnersAmountMax} variant="raised" class="text-xs button-shaped-round min-w-[42px] w-[42px] h-[25px] normal-case ml-2">Max</Button>
          </div>
        </Content>
      </div>
      <Wrapper>
        <div class="w-full">
          <Button disabled={notEvenlySplit} on:click={createGiveaway} ripple={false} class="button-shaped-round w-full py-6 normal-case text-base" variant="raised">Continue</Button>
        </div>
      </Wrapper>
    {/if}
  </div>
