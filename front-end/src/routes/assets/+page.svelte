<script lang='ts'>
	...
	const fetchCards = async () => {
		...
		if (tokenIds.length > 10) {
			...
			while (tokensLeft > 0) {
				...
				metadataPromises.push(MetadataService.getMetadata(slice));
				tokensLeft -= 10;
				offset = chunkSize;
				chunkSize += 10;
			}

			const fetchedCards: Card[] = (await Promise.all(metadataPromises))
				.flat()
				.filter((token => token))
				.map((token) => ({
					...token,
	...
	const onSelect = (event) => {
		...
		cards.update((prev) =>
			prev.map((card) => (card.tokenId === tokenId ? { ...card, selected: !card.selected } : card))
		);
	};
...
{:else if !loading}
	<div class='px-36'>
		...
		<div
			class='border-des-purple rounded-2xl p-4 pb-8 grid gap-y-4 grid-flow-row grid-cols-[repeat(auto-fill,_175px)] justify-center justify-items-center border-2 max-h-[525px] overflow-y-auto'
		>
			{#each $cards as card}
		...
		<div class='mt-4 flex justify-between'>
			<Wrapper>
				<div class='inline-block hover:cursor-pointer'>
					<Button
						...
						variant='raised'
					>
						<span class='text-white'> Create Giveaway </span>
					</Button>
				</div>
				{#if buttonDisabled}
			...
			<Button
				ripple='{false}'
				on:click='{() => goto(`/${$auth.address}`)}'
				class='button-shaped-round'
				variant='raised'
			>
				<span class='text-white'> My Giveaways </span>
			</Button>
		</div>
	</div>
{:else if loading}
